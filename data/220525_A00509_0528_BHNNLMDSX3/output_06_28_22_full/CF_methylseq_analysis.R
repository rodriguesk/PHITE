#analysis of Cell free methylseq data.
...
path <- "/oak/stanford/groups/smontgom/kameronr/CF_DNA_methylation/data/220525_A00509_0528_BHNNLMDSX3/output_06_28_22_full/phiX/lambda_phage/pUC19/GRCh38/"
...
#read in csv file of sample groups
sample_file_path <- "/oak/stanford/groups/smontgom/kameronr/CF_DNA_methylation/data/220525_A00509_0528_BHNNLMDSX3/output_06_28_22_full/NovaSeq_SampleList_20220502.csv"
sample_sheet <- read_csv(file = sample_file_path)

samples_to_analyze <- sample_sheet %>% filter(status == "done")
#"WGM_cfDNA_Pool_"
samples_to_analyze$sample_name_start <- samples_to_analyze$sample %>% gsub(pattern = "L", replacement = "", x = .) %>% paste0("WGM_cfDNA_Pool_",.,"_")
#%>% str_match(string = ., pattern = "(.*)_(.*)") 
...
#str_match(string = list.files(path, full.names=FALSE)[grep("CpG_report",list.files(path, full.names=FALSE))], pattern = "WGM_cfDNA_Pool_(.*_.*)_(S.*)")
sample_order <- str_match(string = coverage_files, pattern = "WGM_cfDNA_Pool_(.*_.*)_(S.*)")[,2] %>% paste0("L",.)

samples_for_analysis_df <- data.frame(sample_path = filter_to_just_samples_to_analyze, sample_id = str_match(string = filter_to_just_samples_to_analyze, pattern = "WGM_cfDNA_Pool_(.*_.*)_(S.*)")[,2] %>% paste0("L",.) )


filter_to_just_samples_to_analyze <- coverage_files[sample_order %in% samples_to_analyze$sample]
ordered_filter_to_just_samples_to_analyze <- filter_to_just_samples_to_analyze

samples_for_analysis_df <- sample_order %>% inner_join(x =sample_sheet,y = samples_for_analysis_df ,by=c("sample"="sample_id"))

file.list <- list(samples_for_analysis_df$sample_path)
...
# myobj <- methRead(list(samples_for_analysis_df$sample_path),
#                   sample.id=list(samples_for_analysis_df$expt_sample_id),
#                   assembly="GRCh38",
...
# myobj <- methRead(as.list(samples_for_analysis_df$sample_path),
#                   sample.id=as.list(samples_for_analysis_df$expt_sample_id),
#                   assembly="GRCh38",
#                   pipeline="bismarkCytosineReport",
#                   treatment=samples_for_analysis_df$condition %>% as.factor() %>% as.numeric(),
#                   context="CpG",
#                   mincov = 1)
...
myobj <- methRead(as.list(samples_for_analysis_df$sample_path),
                  ...
                  assembly="GRCh38",
                  pipeline="bismarkCytosineReport",
                  treatment=samples_for_analysis_df$condition,
                  context="CpG",
                  mincov = 1)

# myobj <- methRead(as.list(samples_for_analysis_df$sample_path[1:2]),
#                   sample.id=as.list(samples_for_analysis_df$expt_sample_id[1:2]),
#                   assembly="GRCh38",
#                   pipeline="bismarkCytosineReport",
#                   treatment=c(0,1),
#                   context="CpG",
